...
"""
function partialschur(C, y;
    nev::Int=min(2, size(C, 1)),
    which::Target=LM(),
    tol::Real=sqrt(eps(real(vtype(C)))),
    mindim::Int=min(max(4, nev), size(C, 1)),
    maxdim::Int=min(max(8, 2 * nev), size(C, 1)),
    restarts::Int=500)
    #s = checksquare(A)
    if nev < 1
    ...
    _partialschur(C, y, vtype(C), mindim, maxdim, nev, tol, restarts, which)
end
...
function _partialschur(C, y, ::Type{T}, mindim::Int, maxdim::Int, nev::Int, tol::Ttol, restarts::Int, which::Target) where {T,Ttol<:Real}
    n = size(C, 1)
    ...
    iterate_arnoldi!(C, y, arnoldi, 1:mindim)

    for iter = 1:restarts

        # Expand Krylov subspace dimension from `k` to `maxdim`.
        iterate_arnoldi!(C, y, arnoldi, k+1:maxdim)
